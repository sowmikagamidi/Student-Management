...
========================================`;
        ...
        window.openUserManagement = async function(schoolId, schoolName) {
            ...
            // Check LMS license before allowing user creation
            const activeLicense = await checkLMSLicense(schoolId);
            const licenseInfo = activeLicense ? {
                class_id: activeLicense.class_id,
                subscription_type: activeLicense.subscription_type === 'D' ? 'Demo' : activeLicense.subscription_type === 'B' ? 'Basic' : 'Premium',
            ...
            // Pre-fill from license if available
            const prefilledClass = licenseInfo && licenseInfo.class_id ? `<option value="${licenseInfo.class_id}" selected>Class ${licenseInfo.class_id}</option>` : classOptions;
            const licenseWarning = !activeLicense ? 
                '<div style="background: #fff3cd; border: 1px solid #ffecb5; padding: 12px; border-radius: 8px; margin-bottom: 15px; color: #856404;"><strong>⚠️ Warning:</strong> No active LMS license with available slots found. You cannot create student users until an LMS license is added.</div>' : 
                `<div style="background: #d4edda; border: 1px solid #c3e6cb; padding: 12px; border-radius: 8px; margin-bottom: 15px; color: #155724;">
                    <strong>✅ Active LMS License Found:</strong><br>
                    Class: ${licenseInfo.class_id}<br>
                    Subscription: ${licenseInfo.subscription_type}<br>
                ...
                <div id="studentSection" style="display:none;">
                    <fieldset>
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                <label>Joining Date</label>
                                <input type="date" id="user_joining" value="${licenseInfo ? licenseInfo.joining_date : ''}">
                            </div>
                            <div class="form-group">
                                <label>Expiry Date</label>
                                <input type="date" id="user_expiry" value="${licenseInfo ? licenseInfo.expiry_date : ''}">
                            </div>
                        </div>
                    </fieldset>
                </div>
            ...
            window.previewCSV = function() {
                ...
                if (!fileInput.files || !fileInput.files[0]) {
                    ...
                    fileError.innerHTML = 'Please select a CSV file to preview';
                ...
                reader.onload = function(e) {
                    ...
                    if (headers.length < 10) {
                        toastr.error('CSV must have at least 10 columns');
                        return;
                    }
            ...
            setTimeout(() => {
                ...
                if (document.getElementById('user_joining') && !document.getElementById('user_joining').value) {
                    document.getElementById('user_joining').value = today;
                }
        ...
        window.openTVSection = async function(schoolId, schoolName) {
            ...
            const createFormHtml = `<form id="createTVForm" novalidate>
                <div class="form-group">
                    ...
                    <input type="text" id="tv_api_key" readonly style="background:#e9ecef; cursor:not-allowed;">
                    <small style="display:block; margin-top:5px; color:#666;">Auto-generated unique API key</small>
                </div>
                <div class="form-group">
                    <label>Class <span class="required">*</span></label>
                            ...
                            ${tvLicences.map(l => {
                                ...
                                return `
                                    <tr style="${isDeleted ? 'background:#fff5f5; opacity:0.7;' : ''}">
                                        ...
                                        <td style="padding:8px;">${new Date(l.expiry_date) > new Date() ? '<span style="color:green;">Active</span>' : '<span style="color:red;">Expired</span>'}</td>
                                        <td style="padding:8px;">
                                            ${!isDeleted ? `<button class="btn btn-info" onclick="viewTVLicence(${l.licence_id})" style="background:#17a2b8; color:white; padding:4px 8px; font-size:12px; margin-right:2px;">👁️ View</button><button class="btn btn-warning" onclick="editTVLicence(${l.licence_id})" style="background:#ffc107; color:#333; padding:4px 8px; font-size:12px; margin-right:2px;">✏️ Edit</button><button class="btn btn-danger" onclick="deleteTVLicence(${l.licence_id})" style="padding:4px 8px; font-size:12px;">🗑️ Delete</button>` : '<span style="color:#666;">Archived</span>'}
                                        </td>
                                    </tr>
                                `;
                            }).join('')}
            ...
            const formHtml = `<div class="toggle-container">
                <button class="toggle-btn active" onclick="showToggleContent('tvCreate')">➕ Create TV Licence</button>
            ...
            setTimeout(() => {
                ...
                if (apiKeyEl && classIdEl) {
                    const generateKeyForClass = () => {
                        apiKeyEl.value = generateAPIKey('tv', classIdEl.value);
                    };
                    generateKeyForClass();
                ...
                if (form) {
                    form.onsubmit = async (e) => {
                        ...
                        if (confirmResult.isConfirmed) {
                            ...
                            const classIdEl = document.getElementById('tv_class_id');
                            const usedStatusEl = document.getElementById('tv_used_status');
                            const joiningDateEl = document.getElementById('tv_joining_date');
                            const expiryDateEl = document.getElementById('tv_expiry_date');
                            const apiKeyEl = document.getElementById('tv_api_key');

                            if (!classIdEl || !usedStatusEl || !joiningDateEl || !expiryDateEl || !apiKeyEl) {
                                Swal.fire({ icon: 'error', title: 'Error', text: 'Form elements not found' });
                                return;
                            }
                            ...
                            const tvRes = await fetch(API_BASE + 'licence_add.php', {
                                ...
                                body: JSON.stringify({
                                    ...
                                    used_status: usedStatusEl.value,
                                    joining_date: joiningDateEl.value,
                                    expiry_date: expiryDateEl.value,
                            ...
                            if (tvResult.success) {
                                Swal.fire({ icon: 'success', title: 'Created!', text: 'TV Licence created successfully' });
...
🔑 API KEY
────────────────────────────────────────
API Key         : ${licence.api_key || 'N/A'}

📊 BASIC INFORMATION
────────────────────────────────────────
Licence ID      : ${licence.licence_id}
Class           : ${licence.class_id || '-'}
Used Status     : ${licence.used_status === 'Y' ? '✓ Yes' : '✗ No'}
Status          : ${status}

📅 DATES
────────────────────────────────────────
Joining Date    : ${licence.joining_date || '-'}
Expiry Date     : ${licence.expiry_date || '-'}

========================================`;

                const viewHtml = `
                    <div style="background:#f8f9fa; padding:20px; border-radius:8px; font-family:monospace; font-size:13px; line-height:1.8; white-space:pre-wrap;">${detailsText.replace(/\n/g, '<br>')}</div>
                    <div style="margin-top: 20px;">
                        <button type="button" onclick="closePanel()" class="btn-submit" style="background:#6c757d;">Close</button>
                    </div>
                `;

                openPanel(`TV Licence #${licenceId}`, viewHtml, null, '500px', '👁️');
        ...
        window.editTVLicence = async function(licenceId) {
            try {
                ...
                const classOptions = classes.map(c => `<option value="${c.class_id}" ${c.class_id == licence.class_id ? 'selected' : ''}>Class ${c.class_id}</option>`).join('');
                ...
                const editFormHtml = `
                    <form id="editTVForm" novalidate>
                        <input type="hidden" id="edit_licence_id" value="${licence.licence_id}">

                        <div class="form-group">
                            <label>API Key</label>
                            <input type="text" id="edit_tv_api_key" value="${licence.api_key || 'N/A'}" readonly style="background:#e9ecef; cursor:not-allowed;">
                            <small style="display:block; margin-top:5px; color:#666;">Cannot be modified</small>
                        </div>
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                <label>Joining Date <span class="required">*</span></label>
                                <input type="date" id="edit_tv_joining_date" value="${formatDate(licence.joining_date)}" required>
                            </div>
                            <div class="form-group">
                                <label>Expiry Date <span class="required">*</span></label>
                                <input type="date" id="edit_tv_expiry_date" value="${formatDate(licence.expiry_date)}" required>
                            </div>
                        </div>
                ...
                openPanel(`Edit TV Licence #${licenceId}`, editFormHtml, null, '500px', '✏️');

                setTimeout(() => {
                    const form = document.getElementById('editTVForm');
                    if (form) {
                        form.onsubmit = async (e) => {
                            e.preventDefault();

                            const classIdEl = document.getElementById('edit_tv_class_id');
                            const usedStatusEl = document.getElementById('edit_tv_used_status');
                            const joiningDateEl = document.getElementById('edit_tv_joining_date');
                            const expiryDateEl = document.getElementById('edit_tv_expiry_date');

                            if (!classIdEl || !usedStatusEl || !joiningDateEl || !expiryDateEl) {
                                toastr.error('Form elements not found. Please try again.');
                                return;
                            }
        ...
        window.deleteTVLicence = async function(licenceId) {
            ...
            if (result.isConfirmed) {
                ...
                else { Swal.fire({ icon: 'error', title: 'Delete Failed', text: data.message }); }
            }
        };
                                    ...
                                    <tbody>
                                        ${licences.map(l => `
                                            <tr>
                                                ...
                                                <td style="padding:8px;">
                                                    <button class="btn btn-danger" onclick="deleteLMSLicence(${l.licence_id})">🗑️ Delete</button>
                                                </td>
                                            </tr>
                                        `).join('')}
                                    </tbody>
